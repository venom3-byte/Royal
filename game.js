...
import {tick,currentPhase,attackUnit,interact} from "./src/systems.js";
...
function openRadial(unit,x,y){const el=document.getElementById("radial");el.classList.remove("hidden");el.style.left=Math.max(80,Math.min(innerWidth-80,x))+"px";el.style.top=Math.max(120,Math.min(innerHeight-120,y))+"px"}
function closeRadial(){document.getElementById("radial")?.classList.add("hidden")}
...
setupInput(canvas,state,{audio,notify,effect,contextTap:()=>{interact(state,{notify,effect});closeRadial();hud.sync(state)},pickUnit:(x,y)=>pickUnit(x,y),openRadial});
...
hud.sync(state);
if(state.running)ui.start.style.display="none";
