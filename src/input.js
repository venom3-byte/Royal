export function setupInput(canvas,state,api){
 let pointer=null,pinchDistance=0,longPress=null;
 ...
 canvas.addEventListener("pointerdown",e=>{api.audio?.init();api.audio?.resume();pointer={x:e.clientX,lastX:e.clientX,lastY:e.clientY,moved:false,id:e.pointerId};canvas.setPointerCapture?.(e.pointerId);clearTimeout(longPress);longPress=setTimeout(()=>{if(pointer&&!pointer.moved){const u=api.pickUnit?.(e.clientX,e.clientY);if(u)api.openRadial?.(u,e.clientX,e.clientY)}} ,520)});
 canvas.addEventListener("pointermove",e=>{if(!pointer)return;const dx=e.clientX-pointer.lastX,dy=e.clientY-pointer.lastY;pointer.lastX=e.clientX;pointer.lastY=e.clientY;if(Math.abs(dx)+Math.abs(dy)>8){pointer.moved=true;clearTimeout(longPress);state.camera.x-=dx/state.camera.zoom*1.05}});
 canvas.addEventListener("pointerup",e=>{const moved=pointer?.moved;clearTimeout(longPress);if(pointer&&!moved&&!document.getElementById("radial")?.classList.contains("hidden")){}else if(pointer&&!moved)api.contextTap?.(e.clientX,e.clientY);pointer=null;canvas.releasePointerCapture?.(e.pointerId)});
