...
export interface PlayerState{x:number;health:number;stamina:number;facing:1|-1;weapon:Weapon;attackTimer:number}
export interface BuildingState{id:string;kind:BuildKind|"castle"|"gate";x:number;level:number;hp:number;maxHp:number}
export interface CropState{id:string;x:number;stage:0|1|2|3;age:number;kind:"wheat"|"winter";water:number}
export interface AnimalState{id:string;kind:"cow"|"sheep"|"bull"|"horse"|"deer"|"bison"|"wolf"|"rabbit";x:number;age:number;state:"graze"|"flee"|"return";wild:boolean}
export interface UnitState{id:string;role:UnitRole;rank:1|2|3|4|5;x:number;hp:number;maxHp:number;state:"idle"|"work"|"train"|"patrol"|"fight"|"escort";homeX:number;targetId?:string}
