...
const KEY="royal-shepherd-save-v2";const BACKUP=KEY+"-backup";
export class SaveManager{
 ...
 load():GameState|null{try{const x=localStorage.getItem(KEY);if(!x)return null;const s=JSON.parse(x) as GameState;if(!s.version||s.version<2)return null;return s}catch{return null}}
 ...
 importJson(raw:string){try{const s=JSON.parse(raw);if(s.version<2)throw new Error("old");localStorage.setItem(KEY,JSON.stringify(s));return true}catch{return false}}
}